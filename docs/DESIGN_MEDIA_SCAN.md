...
- **图片集列表/详情页**（独立子步骤）：路由 `#/galleries`、`#/galleries/:id`，绑定 `FindGalleries` / `GetGallery`；放阶段 5（内容写）或后续。
...
- **图片集页**：独立子步骤，阶段 5（内容写）或后续。
- **阶段 8 插件/scraper**：Gallery NFO 可作为一种元数据来源。
- **阶段 9 性能**：附件按需读取 + 缩略图 + 缓存。
- **阶段 11 重构**：扫描模式抽象可推动 `file/scene/gallery` 解耦。
